...
public:
    ...
    void proces_kreiran(int id_procesa, int trajanje, int prioritet) {
        unique_lock<mutex> l(term_mx);
        cout << "[PROCES] Kreira se proces " << id_procesa << " trajanja " << trajanje << " ms, prioritet: " << prioritet << endl;
    }
    ...
    void rasporedjivac_izabrao(int id_procesa) {
        unique_lock<mutex> l(term_mx);
