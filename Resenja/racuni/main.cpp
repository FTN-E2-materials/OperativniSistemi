...
mutex m; // Propusnica za sprečavanje štetnog preplitanja pri prenosu novca sa jednog računa na drugi
...
retVal prebaci(int izvor, int cilj, double iznos) {
    retVal r;
    // Zaključavamo propusnicu da ne bi druga nit krenula da radi transfer novca dok prethodni nije završen
	unique_lock<mutex> l(m);
    r.staro = racuni[izvor]; // Trenutni iznos na prvom računu
    racuni[izvor] -= iznos; // Umanjimo iznos na prvom računu
    this_thread::sleep_for(chrono::seconds(1)); // Simuliranje trajanja prenosa
    racuni[cilj] += iznos; // Dodamo iznos na drugi račun
    r.novo = racuni[izvor]; // Novo stanje na prvom računu
    return r;
}
